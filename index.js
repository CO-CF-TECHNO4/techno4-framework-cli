...
program
  ...
  .action(async (options) => {
    ...
    if (currentProject) {
      log.text(`${logSymbols.error} Techno4 project already set up in current directory`);
      process.exit(1);
    }
...
program
  ...
  .option('-P, --port <n>', 'Specify UI server port. By default it is 3001', parseInt)
  .description('Generate Techno4 app icons and splash screens')
  .action(async (options) => {
    // Check update
    if (options.skipUpdate === undefined) {
      await checkUpdate();
    ...
    const currentProject = getCurrentProject(cwd);
    if (!currentProject) {
      log.text(`${logSymbols.error} Techno4 project not found in current directory`);
      process.exit(1);
    }
