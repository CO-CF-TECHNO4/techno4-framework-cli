...
const generateTechno4Custom = require('./generate-techno4-custom');
...
module.exports = (options, iconFile) => {
  ...
  // Copy Custom Build
  if (customBuild) {
    const customBuildAssets = generateTechno4Custom(options);
    toCopy.push(
      ...[
        {
          content: customBuildAssets.styles,
          to: path.resolve(cwd, srcFolder, 'css', 'techno4-custom.less'),
        },
        {
          content: customBuildAssets.scripts,
          to: path.resolve(cwd, srcFolder, 'js', 'techno4-custom.js'),
        },
      ],
    );
  }
