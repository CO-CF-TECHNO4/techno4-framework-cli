<template>
  <t4-page name="create">
    ...
    <div class="center-content">
      <t4-block medium-inset strong class="content-block">
        <t4-block-title large>
          <i class="t4-icons block-icon">info_circle</i>
          <span>General</span>
        </t4-block-title>
        <t4-block-title medium>Destination</t4-block-title>
        <t4-block-header
          >New Techno4 app will be created in the following directory.</t4-block-header
        >
        <t4-list no-hairlines-between class="inputs-list">
          <t4-list-input
            type="text"
            outline
            clear-button
        ...
        <t4-block-footer class="display-flex align-items-center"
          ><i style="font-size: 1.5em; margin-right: 8px" class="t4-icons text-color-orange"
            >exclamationmark_triangle_fill</i
          >
          Make sure this folder is empty, the project will be created in the root of this
          folder.</t4-block-footer
        >

        <div class="row">
          <div class="col-25">
            <t4-block-title medium>App Icon</t4-block-title>
            <div class="create-app-icon">
              ...
              <div v-if="iconFile">
                <t4-link @click="resetIcon">Reset to default icon</t4-link>
              </div>
          ...
          <div class="col-75">
            <div class="row">
              ...
              >
                <t4-block-title medium>App (project) name</t4-block-title>
                <t4-list no-hairlines-between class="inputs-list">
                  <t4-list-input
                    type="text"
                    outline
              ...
              >
                <t4-block-title medium>App package (Bundle ID)</t4-block-title>
                <t4-list no-hairlines-between class="inputs-list">
                  <t4-list-input
                    type="text"
                    outline
            ...
            <t4-block-title medium>App Type</t4-block-title>
            <t4-block-header
              >What types of the app are you targeting? (multiple allowed)</t4-block-header
            >
      ...
      <!-- CAPACITOR -->
      <t4-block v-if="type.indexOf('capacitor') >= 0" medium-inset strong class="content-block">
        <t4-block-title large>
          <i class="block-icon block-icon-capacitor"></i>
          <span>Capacitor</span>
        </t4-block-title>

        <t4-block-title>Target Capacitor platform (multiple allowed)</t4-block-title>
        <div class="row checkbox-row">
          <div
      ...
      <!-- CORDOVA -->
      <t4-block v-if="type.indexOf('cordova') >= 0" medium-inset strong class="content-block">
        <t4-block-title large>
        ...
        <t4-block-title>Target Cordova platform (multiple allowed)</t4-block-title>
        <div class="row checkbox-row">
          <div
            class="col-50 medium-25 checkbox-col"
        ...
        <template v-if="cordovaAdvanced">
          <t4-block-title>Pre-installed Cordova plugins</t4-block-title>
          <t4-block-header
            >Will be installed only if <b>iOS</b> or <b>Android</b> platforms
            selected.</t4-block-header
          >
          <t4-list media-list no-hairlines-between>
            <t4-list-item
              checkbox
      ...
      <t4-block medium-inset strong class="content-block">
        ...
        <t4-block-title>What type of framework do you prefer?</t4-block-title>
        <div class="row checkbox-row">
          ...
          <div class="col-50 medium-25 checkbox-col" :class="{ checked: framework === 'vue' }">
            <div
              ...
              "
            >
              <img src="../assets/t4framework-icon-vue.png" />
            </div>
            <div class="col-label">Techno4 with Vue.js</div>
          </div>
          <div class="col-50 medium-25 checkbox-col" :class="{ checked: framework === 'react' }">
            <div
              ...
              "
            >
              <img src="../assets/t4framework-icon-react.png" />
            </div>
            <div class="col-label">Techno4 with React</div>
          </div>
          <div class="col-50 medium-25 checkbox-col" :class="{ checked: framework === 'svelte' }">
            <div
              ...
              "
            >
              <img src="../assets/t4framework-icon-svelte.png" />
            </div>
            <div class="col-label">Techno4 with Svelte</div>
          </div>
        </div>
      </t4-block>
      ...
      <t4-block medium-inset strong class="content-block">
        ...
        <t4-block-title>Choose starter template</t4-block-title>
        <div class="row checkbox-row">
          <div
      ...
      <t4-block medium-inset strong class="content-block">
        ...
        <t4-block-title>Should we setup project with bundler?</t4-block-title>
        <t4-list medium-inset>
          <t4-list-item
        ...
        <template v-if="bundler === 'vite'">
          <t4-block-title>Do you want to setup CSS Pre-Processor?</t4-block-title>
          <t4-list>
            <t4-list-item
      ...
      <t4-block medium-inset strong class="content-block">
        ...
        <t4-list media-list no-hairlines-between>
          <t4-list-item
            title="Include Techno4 Icons and Material Icons icon fonts"
      ...
      <t4-block medium-inset strong class="content-block">
        ...
        <template v-if="!customBuild">
          <p class="text-align-center">
            <i class="t4-icons text-color-orange">exclamationmark_triangle_fill</i><br />Enabling
            custom build will automatically enable Vite bundler with Less pre-processor
          </p>
        </template>
        <template v-if="customBuild">
          <t4-block-title medium>Core components (required)</t4-block-title>
          <div class="row">
            <div class="col-50">
              <t4-list class="no-margin-top" no-hairlines-between>
          ...
          <t4-block-title medium
            >Customizable list of components /
            <a class="link" @click="toggleComponents">Toggle all</a></t4-block-title
          >
          ...
          <t4-block-title medium>CSS</t4-block-title>
          <t4-list no-hairlines-between>
            <t4-list-item
      ...
      <t4-block medium-inset class="no-padding button-block">
        <t4-button
          v-if="!done && !error"
...
export default {
  ...
  watch: {
    'theming.fillBars': function (fillBars) {
      ...
      if (fillBars) {
        t4.$('head').append(self.barsStyleEl);
      } else {
        t4.$(self.barsStyleEl).remove();
      }
  ...
  methods: {
    ...
    onImportInputChange(e) {
      ...
      reader.onload = () => {
        ...
        self.importSettings(content.techno4 || content);
      };
